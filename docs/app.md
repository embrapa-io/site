...
> **Atenção!** Por padrão, as imagens dos serviços que compõem a _stack_ de um _boilerplate_ (no `docker-compose.yaml`) usam a tag `latest`, ou seja, **sem versão fixada**. Isso é proposital: uma aplicação recém-criada parte sempre da versão mais recente das tecnologias que a compõem, e os _boilerplates_ não precisam ser reeditados a cada nova versão publicada. Entretanto, ao evoluir a aplicação para os estágios de publicação (_beta_ e, principalmente, _release_), é fortemente recomendado que a equipe **fixe as versões das imagens** no `docker-compose.yaml` da aplicação, para controlar quando e como as atualizações acontecem. Recomenda-se fixar a **versão maior** (_major_), de modo que a imagem continue recebendo os _patches_ (correções de segurança, desempenho e estabilidade) sem mudar de comportamento por conta própria: por exemplo, `postgres:18` em vez de `postgres:18.6` ou de `postgres` (`latest`). Para **bancos de dados** isso é indispensável, pois uma troca de versão maior pode tornar o volume de dados ilegível pela nova imagem. Fixar a versão maior, porém, **não** protege de um _bug_ introduzido em um _patch_ específico (que pode chegar tanto pela tag `latest` quanto por `postgres:18`); nesses casos o caminho é avançar para o _patch_ corrigido. A decisão de fixar ou não, e em que granularidade, é da equipe, no âmbito de cada aplicação. A orientação completa para _builds_ em produção (o que fixar, como manter atualizado e como reverter) está no capítulo sobre [publicação de aplicações]({{ site.baseurl }}/docs/delivery#images).

> **Atenção!** Na plataforma, a atualização das imagens **não** acontece ao publicar uma nova _tag_: o _deploy_ reutiliza as imagens já presentes no cluster. Para trazer versões novas (de imagens em `latest` ou um _patch_ mais recente de uma versão fixada), utilize o **_restart_ da _build_ com a opção "ATUALIZAR as imagens dos containers"** na [_dashboard_](https://dashboard.embrapa.io), ciente de que **todas** as imagens sem versão fixada na _stack_ serão atualizadas de uma só vez. Retornar a uma versão anterior exige fixar explicitamente a _tag_ antiga no `docker-compose.yaml` e publicar uma nova _tag_ da aplicação; para bancos de dados, em geral, não é possível.
