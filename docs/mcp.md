...
- [MCP do SEG](#seg): provê _tools_ para consultar dados corporativos do **Sistema Embrapa de Gestão** (projetos, planos, atividades), do catálogo **CatSoft** (softwares), do **portal embrapa.br** (notícias, palavras-chave) e da base de **empregados** da Empresa.
...
> **Dica!** Os quatro MCPs podem ser utilizados simultaneamente. Com todos conectados, é possível consultar a configuração de uma _build_ no Dashboard, verificar os logs no Loki, criar uma _issue_ no Kanban e ainda buscar projetos do SEG ou perfis de empregados — tudo em uma única conversa.
...
A configuração segue o mesmo padrão descrito na seção [Configuração em IDEs e CLIs](#config) do MCP do Dashboard. Basta substituir a URL por `https://seg.mcp.embrapa.io`. Exemplos:

**Claude Desktop ou Web** — Configurações → Conectores → Adicionar conector personalizado → informar `https://seg.mcp.embrapa.io`.
...
```bash
gemini mcp add -s user -t http seg https://seg.mcp.embrapa.io
```
...
```json
"seg": {
  "serverUrl": "https://seg.mcp.embrapa.io"
}
```
