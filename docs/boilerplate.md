...
> **Atenção!** O nome do arquivo gerado pelo serviço `backup` (`${IO_PROJECT}_${IO_APP}_${IO_STAGE}_${IO_VERSION}_$$(date +'%Y-%m-%d_%H-%M-%S').tar.gz`, na raiz do volume montado em `/backup`) é um **padrão obrigatório** da plataforma — [veja as regras completas](#cli:backup). Cuidado também com aspas duplas dentro de `sh -c "..."` (quebram o comando) e com o `-exec {} \\;` do `find`, que precisa ser escrito `\\;` no `docker-compose.yaml`; para _scripts_ mais longos, prefira `entrypoint: ["/bin/sh", "-c"]` com `command:` como **lista de um único item** em bloco literal (`command:` seguido de `- |`); um `command: |` sem a lista é quebrado em argumentos pelo Docker Compose.
